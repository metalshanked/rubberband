...
## 10. agent_admin Application Layout

The `agent_admin` web app uses the design system above as a productive administration surface rather than a marketing page. The first screen is the working console: a dark 48px masthead, tabbed navigation, live connection status, inventory counts, and a persistent settings panel for administrators.

The product-facing name is **Agent Command Center**. The implementation directory can remain `agent_admin`, but user-facing UI text and the browser tab should avoid the underscore form. The masthead uses a small geometric blue mark to make the tool recognizable without adding decorative clutter.

The application layout keeps the runnable app directly under `agent_admin`:
- `agent_admin.py` is the process entrypoint.
- `agent_admin.py` also assembles FastAPI, AuthBlue middleware, static UI mounting, and startup.
- `api/routes.py` owns HTTP route handlers.
- `core/config.py` owns environment, persisted settings, and UI path configuration.
- `core/schemas.py` owns request models.
- `elastic/client.py` owns Kibana and Elasticsearch HTTP transport.
- `elastic/resources.py` owns Agent Builder, workflow, connector, and access-plan compatibility logic.
- `ui/` owns static browser assets.
- `tests/` owns live smoke and contract test scripts.

### Primary tabs
- Overview: connection health for Kibana and Elasticsearch, selected space, and live counts for spaces, agents, tools, and workflows.
- Spaces & Access: a compact table of Kibana spaces, a create/update form using the official Spaces API, a visible target-space summary, and the LDAP/AD access planner.
- Agent Builder: a space selector and live lists for agents, tools, skills, and conversations from `/api/agent_builder`.
- Workflows: a space selector, workflow inventory, active execution polling, and kill-switch controls from `/api/workflows`.
- Connectors: a space-aware management view for Kibana action connectors from `/api/actions/connectors`, with edit, delete, and cross-space copy operations.
- Access planning lives with space management so administrators can see which Kibana spaces a role will apply to before previewing or applying changes.

### Settings panel
- The settings panel is hidden by default and only opens when the administrator presses the Settings button.
- The settings panel remains visible on desktop as a right-side administration rail.
- The panel stores Kibana URL, Elasticsearch URL, API keys, optional basic auth, the master insecure TLS flag, and request timeout.
- Secret fields are never re-rendered with their stored values. Blank secret fields keep the existing saved value.
- TLS can be set to insecure with `AGENT_ADMIN_TLS_INSECURE=true` or the Settings panel. This disables certificate verification for Kibana, Elasticsearch, and AuthBlue calls.
- Debug logging is enabled with `AGENT_ADMIN_DEBUG_LOG=true`; it logs outbound request method, target, path, status, elapsed time, and failures without logging credentials or request bodies.
- The app can run directly with `python agent_admin.py`; startup is controlled by environment variables only.
- `AGENT_ADMIN_BASE_PATH` moves both the UI and API under a shared URL prefix.
- Settings supplied through server environment variables are locked in the UI and cannot be changed by browser users. This keeps server-only values such as internal Elasticsearch URLs and service credentials out of browser-controlled configuration.
- AuthBlue authorization is enabled when `AUTHBLUE_USERINFO_URL` and `AUTHBLUE_ALLOWED_ADMIN_GROUPS` are configured. The app validates the `bluetoken` cookie against AuthBlue and requires membership in one of the configured groups. `AGENT_ADMIN_AUTH_DISABLED=true` disables this check for local use.

### Access planner behavior
- The user chooses an LDAP/AD group DN, one or more spaces, a layman-level preset, and index patterns.
- Elasticsearch role mappings use `rules.field.groups` with the LDAP/AD group DN value. The `dn` rule field is reserved for matching a user's own distinguished name.
- Auto-generated access object names are readable by default, for example `access_space1_viewer_app-team` and `access_map_space1_app-team`; users can still override both names.
- The Access tab starts with a target-space strip that always shows the spaces currently selected for role generation.
- The spaces table is the single place to choose target spaces. Checked rows are included in the generated role payload.
- Space details are edited from the same table with a single Details action, and the form can be cleared to enter a new space.
- Space IDs are unique and are read-only when editing an existing space. The Clone action stages a new editable Space ID and name without creating anything until Save space.
- Cloning a space creates the new Kibana space and clones mapped access roles and role mappings from the source space to the new target space.
- New or updated spaces are saved before the access workflow continues, and the saved space is automatically checked for access.
- Toast messages auto-dismiss after a short delay.
- The access-level selector includes a dynamic plain-language info box explaining what the selected preset grants before preview/apply.
- Data index access is selected separately from Kibana app access. The choices are read only, write existing indices, auto-create and write, or manage matching indices.
- The Access tab is organized as a horizontal three-step workflow. Only the current step panel is visible: create or edit access, confirm spaces and permissions, then map the role to an AD group.
- The selected spaces are written directly into the Kibana role payload under `kibana[].spaces`, so the mapped AD group only receives the configured feature permissions in those spaces. Elasticsearch index privileges are scoped separately by the configured index patterns.
- A Clear action resets the planner fields, selected spaces, summary, and raw preview.
- Low-level feature IDs such as `agentBuilder` and `workflowsManagement` are treated as implementation defaults and are not shown in the normal UI.
- Existing Kibana roles and Elasticsearch role mappings can be loaded into the form, edited, previewed, and applied back to the same names.
- The UI previews the generated Kibana role and Elasticsearch role mapping before apply.
- Destructive actions for spaces, connectors, roles, and role mappings use subtle red outline buttons and an in-app confirmation modal. Deletes require exact-name confirmation.
- Presets map to Elastic privileges as follows:
  - Use and review: `agentBuilder` read, `workflowsManagement` read, Actions read, index read and metadata.
  - Build agents and workflows: `agentBuilder` all, `workflowsManagement` all, Actions all, index read and metadata.
  - Operate workflows: `agentBuilder` read, `workflowsManagement` read plus `workflow_update` and `workflow_execution_cancel`, Actions read, index read and metadata.
  - Space administrator: Kibana base all in selected spaces, while Elasticsearch index access remains scoped to the configured patterns.
- Data index access maps to Elasticsearch index privileges as follows:
  - Read only: `read`, `view_index_metadata`.
  - Write existing indices: `read`, `write`, `view_index_metadata`.
  - Auto-create and write: `auto_configure`, `create_index`, `create`, `write`, `read`, `view_index_metadata`.
  - Manage matching indices: `manage`, `write`, `read`, `view_index_metadata`.
- The workflow feature key is `workflowsManagement` in the live 9.4.x feature list; older saved roles that used `workflows` are still recognized when loading existing roles.

### Connector management behavior
- The Connectors tab lists connectors in the selected source space and loads connector types from `/api/actions/connector_types`.
- Editing uses `GET /api/actions/connector/{id}` and `PUT /api/actions/connector/{id}`. Creating uses `POST /api/actions/connector/{id}`.
- Deleting uses `DELETE /api/actions/connector/{id}` and requires browser confirmation because Kibana cannot recover deleted connectors.
- Copying selected connectors reads source metadata and config, then creates or updates connectors in the target space. Conflicts are detected by connector ID and, optionally, by connector name.
- Secrets are not copied automatically because Kibana does not return sensitive connector values. The UI provides a manual `secrets_by_id` JSON override for copied connectors and a secrets JSON field for create/update.
- Preconfigured connectors and system action connectors are skipped during copy because they are managed outside normal saved connector CRUD.

### Kill switch behavior
- The Workflows tab implements the architecture in `docs/agent-workflow-kill-switch-architecture.md`.
- "Live poll" refreshes active workflow execution state every 5 seconds. This is polling, not a server push stream, because the public Workflows API exposes paginated execution listing.
- Active execution polling calls `GET /api/workflows/workflow/{workflowId}/executions` with statuses `running`, `pending`, `waiting`, and `waiting_for_input`.
- A workflow kill action first calls `PUT /api/workflows/workflow/{id}` with `enabled: false`, then calls `POST /api/workflows/executions/{executionId}/cancel` for active executions.
- Individual executions can be cancelled from the table without disabling the workflow.
- Direct Agent Builder executions cannot be centrally aborted through a documented public API, so autonomous agents should run through workflow wrappers when this kill switch is required.

### Agent detail behavior
- The Agent Builder tab uses a single agent dropdown and a focused detail panel instead of showing all agents, tools, and skills in the main view.
- Detail descriptions use small gray text with regular weight, avoiding bold labels inside dense agent/tool/skill lists.
- Raw API responses live behind collapsed "Show raw JSON" disclosure controls on each tab and are not open by default.
- Elastic 9.4.x exposes the technical preview Agent Builder `visibility` field with `public`, `shared`, and `private` values. The app edits this official field directly in the selected-agent panel.
- Visibility controls per-agent view/edit behavior inside Agent Builder, while spaces and roles still control who has any Agent Builder access in the first place.
- The app derives tool IDs from `configuration.tools` and joins those IDs to the tool inventory from `GET /api/agent_builder/tools`.
- The app derives skill IDs from `configuration.skills` or related skill ID fields and joins those IDs to `GET /api/agent_builder/skills` when available.
- Plugins are loaded from the technical preview `GET /api/agent_builder/plugins` endpoint when available.
- Plugin and extension references are displayed when present in the agent configuration, and plugins are also inferred when an installed plugin owns a skill assigned to the selected agent.
- Workflow membership is inferred by scanning workflow definitions and metadata for the selected agent ID. This catches common `ai.agent` workflow wrapper usage without relying on undocumented workflow internals.
